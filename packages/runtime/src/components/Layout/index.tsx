...
export default (props: any) => {
  ...
  const apps = user?.systemList.map(sys => ({ name: sys.systemName, code: sys.systemCode, path: `/${sys.systemCode}` }));
  ...
  return (
    ...
    >
      ...
      logo={logo}
        ...
        menuItemRender={(item: { path: any }, dom) => <Link to={`${item.path}`}>{dom}</Link>}
        rightContentRender={() => <RightContent currentUser={user}></RightContent>}
        {...settings}
        navTheme="light"
      >
        {children}
