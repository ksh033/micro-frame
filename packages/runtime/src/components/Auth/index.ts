...
const getStorage = <T>(skey:string):T|null => {
 ...
 const setUser = (user: User) => {
  ...
  if (!userAppInfos[currentUser.userAppInfo.currentSystem.systemCode]) {
    userAppInfos[currentUser.userAppInfo.currentSystem.systemCode] = user.userAppInfo;
  ...
  if (!currentUser) {
    ...
    if (!userAppInfos) {
     ...
     if (appCode && userAppInfos && currentUser) {
        ...
        if (tuserApp) {
          if (tuserApp.currentSystem.systemCode !==
...
export {
  ...
  updateUser,getUser,changeApp,setUser
}