 ...
 */
 import { extend, RequestMethod, RequestOptionsInit, RequestResponse, Context, RequestOptionsWithResponse, RequestOptionsWithoutResponse } from 'umi-request';
 ...
 const getRequestMethod = () => {
     ...
     errorHandler: (error: RequestError) => {
       ...
       if (errorInfo) {
         ...
         const errorPage = DEFAULT_ERROR_PAGE
           ...
           const errormsg = errorMessage || errorShowTip
   ...
   requestMethodInstance.interceptors.request.use((url: string, options:RequestOptionsInit &
     { skipErrorHandler?: boolean } & NewRequestOptionsInit) => {
     const user = getUser();
    const headers: any = { version: '1.0', ...options.headers };
     ...
     // 处理图片上传
     if (options.method && options.method.toUpperCase() === ExtendMethod.FILEUPLOAD) {
       ...
       for (const item in dataParament) {
         if (item !== 'fileLists') {
           if (item !== null && dataParament[item] !== undefined && dataParament[item] !== null) {
   ...
   // 文件下载处理
   requestMethodInstance.interceptors.response.use(response => {
 ...
 export interface RequestMethodInUmi<R = false> {
   <T = any>(
     url: string,
     options: RequestOptionsWithResponse & { skipErrorHandler?: boolean } & NewRequestOptionsInit,
   ): Promise<RequestResponse<T>>;
   <T = any>(
     url: string,
     options: RequestOptionsWithoutResponse & { skipErrorHandler?: boolean }&NewRequestOptionsInit,
   ): Promise<T>;
   <T = any>(
     url: string,
     options?: RequestOptionsInit & { skipErrorHandler?: boolean } & NewRequestOptionsInit,
